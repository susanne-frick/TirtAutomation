...
tirt.mplus.syntax <- function(design.load, names.pairs, item.short, id.var, file.inp=NULL, file.data, title, out.command, fscores.file, missings.as) {
  ...
  #starting values for correlations between traits (set to 0)
  trait.cor <- NULL
  ...
  if (ncol(design.load)>1) {
    tirt.model <- paste(ch.trait.loadings, "! means for all traits are set to 0",ch.m.traits,
                        ...
                        "! starting values for correlations between traits are set to 0",ch.trait.cor,
  ...
  syntax.tirt <-  list(paste0("TITLE: ",title,";"),
                       ...
                       paste0("VARIABLE: \nNames ARE \n", id.var, "\n", ch.names.pairs,";\n",
                              "USEVARIABLES ARE ", names.pairs[1],"-",names.pairs[length(names.pairs)],";",
                              "\nCATEGORICAL ARE ALL;", "\nIDVARIABLE = ", id.var, ";\n",
